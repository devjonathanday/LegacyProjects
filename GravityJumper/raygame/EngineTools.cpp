...
{
	DrawTexture(tex, screenOffset.x + rec.x - cameraOffset.x, screenOffset.y + rec.y - cameraOffset.y, color);
}
...
void ParticleSystem::Draw(Vector2 cameraOffset, Vector2 screenOffset)
{
	for (int i = 0; i < particleGroup.size(); i++)
		particleGroup[i].Draw(cameraOffset, screenOffset);
}